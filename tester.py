...
#    Updated: 2023/04/13 23:08:02 by adi-stef         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def ft_check(ntests, nargv, tosave, checker=C):
    ...
    print(f"tests: {b}{tests}{e} - ok: {g}{len(moves)}{e} - ko: {r}{ko}{e} - numbers: {nargv} - max: {max(moves)} - min: {min(moves)} - avg: {sum(moves) // len(moves)} - time: {time}")
...
{b}python3 tester.py{e} {g}[number_of_tests] [stack_size]{e}""")
    ...
    if (len(argv) == 2 or (len(argv) == 3 and str(argv[1]) in ['m', 'b', 'a'])):
        if (len(argv) == 3 and not access(argv[2], X_OK)):
            print(f"{rb}Error{e}\n{r}Path '{argv[2]}' not found or doesn't have permissions{e}")
            exit(1)
        if (argv[1] == 'm'):
            ft_check_m()
        elif (argv[1] == 'b'):
            if (not access(CB, X_OK)):
                print(f"{rb}Error\n{e}{r}Path '{CB}' not found or doesn't have permissions{e}")
                exit(1)
            ft_check_b()
        elif (argv[1] == 'a'):
            ft_check_m()
        ...
        try:
            ...
            exit(1)
        if (len(argv) == 4 and not access(argv[3], X_OK)):
            print(f"{rb}Error{e}\n{r}Path '{argv[3]}' not found or doesn't have permissions{e}")
            exit(1)
